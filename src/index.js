...
function createSidebar(name) {
	...
	${config.site.motd}
	<hr>
	<form onsubmit="searchSubmit('${all_topics}', document.getElementById('wiki-search').value); return false;">
	<input class="u-full-width" type="search" placeholder="Search for a page..." id="wiki-search">
	</form>
	<p id="results"></p>
	<hr>
...
function addLinks(html, current_page) {
	all_topics.forEach(topic => {
		if (topic != current_page) {
			...
			html = html.replaceAll(' ' + topic + '.', ` <a href="${topic}.html">${topic}</a>.`);
			html = html.replaceAll('<li>' + (topic) + '</li>', `<li><a href="${topic}.html">${(topic)}</a></li>`);
			html = html.replaceAll('<li>' + format_title(topic) + '</li>', `<li><a href="${topic}.html">${format_title(topic)}</a></li>`);
		}
	});
	return html;
}
...
function moveStaticFiles() {
	console.log("Copying static css / scripts...");

	// CSS
	fs.readdir('css', (err, files) => {
		files.forEach(file => {
			fs.copyFileSync(process.cwd() + "/css/" + file, output_dir + "/css/" + file)
	...
	fs.readdir('scripts', (err, files) => {
		files.forEach(file => {
			fs.copyFileSync(process.cwd() + "/scripts/" + file, output_dir + "/scripts/" + file)
			console.log("Copied: " + file);
		  });
	});
}
...
class MaggieCommand extends Command {
  async run() {
	...
	if (flags.build) {
		...
		if (!fs.existsSync(output_dir + '/scripts')){
